...
W1 = tf.Variable(tf.random_uniform([5, 3], -1.0, 1.0))
W2 = tf.Variable(tf.random_uniform([4, 5], -1.0, 1.0))
b2 = tf.Variable(tf.random_uniform([4], -1.0, 1.0))
W3 = tf.Variable(tf.random_uniform([1, 4], -1.0, 1.0))
b3 = tf.Variable(tf.random_uniform([1], -1.0, 1.0))

l1 = tf.div(1., 1. + tf.exp( - (tf.matmul(W1, X))))
l2 = tf.div(1., 1. + tf.exp( - (tf.matmul(W2, l1) + b2)))
#y_ = tf.sigmoid(tf.matmul(W2, l1) + b2)
y_ = tf.div(1., 1. + tf.exp( - (tf.matmul(W3, l2) + b3)))
